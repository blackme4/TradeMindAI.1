...
// MT5 API
export const linkMT5Account = async (accountData) => {
  try {
    const response = await client.post('/mt5/link', accountData);
    return response.data;
  } catch (error) {
    console.error('MT5 link error:', error);
    throw error;
  }
};
...
export const getMT5Status = async () => {
  try {
    const response = await client.get('/mt5/status');
...
export const unlinkMT5Account = async () => {
  try {
    const response = await client.delete('/mt5/unlink');
    return response.data;
  } catch (error) {
    console.error('MT5 unlink error:', error);
    throw error;
  }
};

export const toggleMT5Sync = async (enabled) => {
  try {
    const response = await client.put('/mt5/sync', { enabled });
    return response.data;
  } catch (error) {
    console.error('MT5 sync toggle error:', error);
    throw error;
  }
};

export const manualSyncMT5 = async () => {
  try {
    const response = await client.post('/mt5/manual-sync');
    return response.data;
  } catch (error) {
    console.error('MT5 manual sync error:', error);
    throw error;
  }
};

export const getMT5Brokers = async () => {
  try {
    const response = await client.get('/mt5/brokers');
    return response.data.data;
  } catch (error) {
    console.error('MT5 brokers error:', error);
    throw error;
  }
};
